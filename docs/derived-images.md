...
- **The container entrypoint is optional.** In the container flavors it hydrates the image-managed home files, mirrors bb's log files to stdout, and runs bb with SIGTERM forwarding for a clean stop. A derived container image that overrides the command and bypasses it skips hydration and log mirroring (bb writes its service output to files and never to stdout). The VM flavors do not use this entrypoint: `bb.service` runs hydration through `ExecStartPre` and systemd owns shutdown. In either case, the image home is pre-hydrated at build time, so a new home starts with the managed files; ongoing updates require either the entrypoint, the service, or an explicit `hydrate-home.sh install` call.
...
For a derived container image that bypasses the baked command, `bb-app` alone is enough to serve: hydration is the entrypoint's only state-mutating step, and log mirroring is a convenience rather than plumbing bb needs. `~/.bb/logs/*` files still show up wherever the derived image points at them. For a VM derivation, extend or override `bb.service` instead; keep `User=developer` and the hydration `ExecStartPre` unless you deliberately replace those contracts.
