...
- `build/` — host-side verification tooling: `build/check.sh` is the `make check` runner; it sources `build/check/lib.sh` for the shared helpers and runs everything in `build/check/` in lexical order (one file per concern, each independently shellchecked). `build/fcshim.c` is the LD_PRELOAD interposer that turns the fontconfig "no writes on the system cache" claim into a release gate: make check compiles it fresh, proves its path-resolving wrappers are active, drives two Chromium launches with it, and refuses the release on any observed fontconfig-related write outside the accepted xdg-cache and cold `.uuid` classes, or on a warm-cache launch that writes anything at all.
